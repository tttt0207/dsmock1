...
def build_scene_result(frame, frame_candidates, area_stabilizers: dict | None = None) -> SceneBuildResult:
...
def build_scene_detections(frame, frame_candidates, area_stabilizers: dict | None = None) -> list[SceneDetection]:
...
def select_target_for_task(
	...
	if task_id == protocol.TASK_ADV_2_2:
		if not configs:
			return None
		shape_name = protocol.SHAPE_ID_TO_NAME.get(configs[0].shape_id)
		matches = [
			item
			for item in sorted_detections
			if _matches_shape(item, shape_name)
		]
		if not matches:
			return None
		return matches[0]
	...
	if task_id == protocol.TASK_ADV_2_3:
		if target_index < 0 or target_index >= len(configs):
			return None
		config_item = configs[target_index]
