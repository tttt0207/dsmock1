...
class VisionTaskController:
	...
	def _handle_config(self, frame: protocol.Frame) -> None:
		...
		if shape_id not in protocol.SHAPE_ID_TO_NAME or color_id not in protocol.COLOR_ID_TO_NAME:
			self.state = TaskState.ERROR
			self._log(
		...
		if self.current_task in (protocol.TASK_ADV_2_2, protocol.TASK_ADV_2_3) and last_flag == 0x01:
	...
	def send_target_angle(self, angle_deg: float) -> bytes:
		frame = protocol.build_angle_frame_from_deg(angle_deg)
		if config.SERIAL_LOG_ENABLED:
			print(f"[SERIAL] send angle frame: {protocol.bytes_to_hex(frame)}")
		self.sender(frame)
		return frame
	...
	def send_target_coordinate(self, x_cm: float, y_cm: float) -> bytes:
		...
		if config.SERIAL_LOG_ENABLED:
			print(f"[SERIAL] send coordinate frame: {protocol.bytes_to_hex(frame)}")
		self.sender(frame)
		self.state = TaskState.WAIT_NEXT_HOME
		return frame
