...
class VisionTaskStatus:
	...
	gripper_angle_deg: float | None = None
	message: str = ""
...
class VisionTaskRunner:
	...
	def _update_target(self, target: target_selector.SceneDetection) -> target_selector.SceneDetection | None:
		...
		standard_x_values = [
			item.standard_center_x
			for item in self.target_history
			if item.standard_center_x is not None
		]
		standard_y_values = [
			item.standard_center_y
			for item in self.target_history
			if item.standard_center_y is not None
		]
		...
		object_axis_angle_deg = self._stable_object_axis_angle()

		if (
			latest.shape != "ball"
			and object_axis_angle_deg is None
		):
			self.target_history = self.target_history[-1:]
			self.status.message = "target angle unavailable"
			return None
		...
		return target_selector.SceneDetection(
			...
			standard_center_x=(
				float(statistics.median(standard_x_values))
				if standard_x_values
				else latest.standard_center_x
			),
			standard_center_y=(
				float(statistics.median(standard_y_values))
				if standard_y_values
				else latest.standard_center_y
			),
	...
	def _stable_object_axis_angle(self) -> float | None:
		latest = self.target_history[-1]

		if latest.shape == "ball":
			return None

		angle_values = [
			item.object_axis_angle_deg
			for item in self.target_history
			if item.object_axis_angle_deg is not None
		]

		if len(angle_values) != len(self.target_history):
			return None

		period = 90.0 if latest.shape == "cube" else 180.0
		reference = float(angle_values[-1])
		unwrapped = [
			reference + self._periodic_delta(float(angle), reference, period)
			for angle in angle_values
		]

		if max(unwrapped) - min(unwrapped) > config.MAX_OBJECT_AXIS_JITTER_DEG:
			self.target_history = self.target_history[-1:]
			self.status.message = "target angle jitter too large"
			return None

		return coordinate.normalize_angle_180(float(statistics.median(unwrapped)))

	@staticmethod
	def _periodic_delta(angle_deg: float, reference_deg: float, period_deg: float) -> float:
		return (float(angle_deg) - float(reference_deg) + period_deg / 2.0) % period_deg - period_deg / 2.0
	...
	def _send_stable_target(self, target: target_selector.SceneDetection, controller: task_state.VisionTaskController) -> None:
		...
		base_angle_deg = coordinate.calculate_base_angle_deg(x_cm, y_cm)
		gripper_angle_deg = coordinate.calculate_gripper_angle_deg(
			target.object_axis_angle_deg,
			base_angle_deg,
			target.shape,
		)
		x_send = x_cm + config.ARM_X_OFFSET_CM
		y_send = y_cm + config.ARM_Y_OFFSET_CM
		controller.record_completed_target(target.global_center_x, target.global_center_y)
		self.status.state_text = "sent"
		self.status.x_cm = x_send
		self.status.y_cm = y_send
		self.status.object_axis_angle_deg = target.object_axis_angle_deg
		self.status.base_angle_deg = base_angle_deg
		self.status.gripper_angle_deg = gripper_angle_deg
		...
		self._log(
			"[COORD]\n"
			f"raw: x={x_cm:.2f} cm, y={y_cm:.2f} cm\n"
			f"offset: x={config.ARM_X_OFFSET_CM:+.2f} cm, y={config.ARM_Y_OFFSET_CM:+.2f} cm\n"
			f"send: x={x_send:.2f} cm, y={y_send:.2f} cm"
		)
		self._log(
			"[ANGLE] "
			f"shape={target.shape}, "
			f"object_axis={self._format_angle(target.object_axis_angle_deg)}, "
			f"base={base_angle_deg:.2f} deg, "
			f"gripper={gripper_angle_deg:.2f} deg"
		)
		tx_angle_deg = coordinate.normalize_gripper_angle(
			gripper_angle_deg + config.GRIPPER_TX_ANGLE_OFFSET_DEG
		)
		self._log(
			"[ANGLE_TX] "
			f"raw_gripper={gripper_angle_deg:.2f} deg, "
			f"offset={config.GRIPPER_TX_ANGLE_OFFSET_DEG:+.2f} deg, "
			f"tx={tx_angle_deg:.2f} deg"
		)
		controller.send_target_angle(tx_angle_deg)
		controller.send_target_coordinate(x_send, y_send)
		self._log(
			"[STAGE] "
	...
	@staticmethod
	def _format_angle(angle_deg: float | None) -> str:
