...
def normalize_gripper_angle(angle_deg: float) -> float:
	normalized = normalize_angle_180(angle_deg)
...
def calculate_base_angle_deg(x_cm: float, y_cm: float) -> float:
	import math

	return normalize_angle_180(math.degrees(math.atan2(float(y_cm), float(x_cm))))


def calculate_gripper_angle_deg(
	object_axis_angle_deg: float | None,
	base_angle_deg: float,
	shape: str | None,
) -> float:
	if shape == "ball" or object_axis_angle_deg is None:
		return 0.0

	if shape == "cube":
		candidates = [
			normalize_gripper_angle(float(object_axis_angle_deg) - float(base_angle_deg)),
			normalize_gripper_angle(float(object_axis_angle_deg) + 90.0 - float(base_angle_deg)),
		]
		return min(candidates, key=lambda item: abs(item))

	return normalize_gripper_angle(float(object_axis_angle_deg) - float(base_angle_deg))
...
def frame_point_to_arm_coordinate(
	frame_x: float,
	frame_y: float,
	placement,
	...
	base_angle_deg = calculate_base_angle_deg(x_cm, y_cm)
	gripper_angle_deg = calculate_gripper_angle_deg(
		object_axis_angle_deg,
		base_angle_deg,
		shape,
	)
