...
class ProtocolParserTests(unittest.TestCase):
	...
	def test_negative_angle_pack_uses_angle_frame_head_tail(self):
		frame = protocol.build_angle_frame_from_deg(-45.0)
		self.assertEqual(frame, bytes([0x04, 0xEE, 0x6C, 0x00, 0x00, 0x40]))

	def test_positive_angle_pack_uses_angle_frame_head_tail(self):
		frame = protocol.build_angle_frame_from_deg(45.0)
		self.assertEqual(frame, bytes([0x04, 0x11, 0x94, 0x00, 0x00, 0x40]))
...
class StateMachineTests(unittest.TestCase):
	...
	def test_new_task_select_byte_mapping(self):
		expected = [
			(0x11, protocol.TASK_BASIC_1_1, 1),
			(0x22, protocol.TASK_BASIC_1_2, 1),
			(0x33, protocol.TASK_BASIC_1_3, 4),
			(0x44, protocol.TASK_ADV_2_1, 3),
			(0x55, protocol.TASK_ADV_2_2, 0),
			(0x66, protocol.TASK_ADV_2_3, 0),
		]

		for raw_task_id, expected_task_id, expected_count in expected:
			controller, _, _ = self.make_controller()
			controller.handle_frame(task_frame(raw_task_id))
			self.assertEqual(controller.current_task, expected_task_id)
			self.assertEqual(controller.target_count, expected_count)

	def test_new_config_keys_save_shape_and_color_ignore_position_for_matching(self):
		controller, sent, _ = self.make_controller()
		controller.handle_frame(task_frame(0x55))
		controller.handle_frame(config_frame(0x01, 0x99, 0x05, 0x41))
		self.assertEqual(sent, [])
		self.assertEqual(controller.target_count, 1)
		self.assertEqual(controller.task_config_queue[0].slot_id, 0x99)
		self.assertEqual(protocol.SHAPE_ID_TO_NAME[controller.task_config_queue[0].shape_id], "cube")
		self.assertEqual(protocol.COLOR_ID_TO_NAME[controller.task_config_queue[0].color_id], "pink")
		self.assertEqual(
			target_selector.expected_detection_hint(
				protocol.TASK_ADV_2_2,
				0,
				controller.task_config_queue,
			),
			("cube", "pink"),
		)
	...
	def test_advanced_2_2_third_home_exits(self):
		controller, _, clock = self.make_controller()
		controller.handle_frame(task_frame(protocol.TASK_ADV_2_2))
		controller.handle_frame(config_frame(0x00, 1, protocol.SHAPE_BALL, protocol.COLOR_ANY))
		controller.handle_frame(config_frame(0x01, 2, protocol.SHAPE_CUBE, protocol.COLOR_BLUE))
		self.assertEqual(controller.current_target_index, 0)
	...
	def test_send_angle_does_not_change_coordinate_protocol(self):
		controller, sent, _ = self.make_controller()
		controller.handle_frame(task_frame(protocol.TASK_BASIC_1_1))
		angle_frame = controller.send_target_angle(-45.0)
		coord_frame = controller.send_target_coordinate(-3.30, -4.10)
		self.assertEqual(sent, [angle_frame, coord_frame])
		self.assertEqual(angle_frame, bytes([0x04, 0xEE, 0x6C, 0x00, 0x00, 0x40]))
		self.assertEqual(coord_frame, bytes([0x03, 0xFE, 0xB6, 0xFE, 0x66, 0x30]))

	def test_gripper_tx_angle_offset_normalizes_before_send(self):
		tx_angle = coordinate.normalize_gripper_angle(
			43.79 + config.GRIPPER_TX_ANGLE_OFFSET_DEG
		)
		frame = protocol.build_angle_frame_from_deg(tx_angle)
		self.assertAlmostEqual(tx_angle, -46.21, places=2)
		self.assertEqual(frame, bytes([0x04, 0xED, 0xF3, 0x00, 0x00, 0x40]))
...
class TargetSelectionTests(unittest.TestCase):
	...
	def test_advanced_2_2_any_color_shape(self):
		...
		target = target_selector.select_target_for_task(protocol.TASK_ADV_2_2, 0, items, configs)
		self.assertEqual(target.global_center_x, 10)

	def test_advanced_2_2_reselects_current_leftmost_after_first_removed(self):
		configs = [
			task_state.TargetConfig(slot_id=7, shape_id=protocol.SHAPE_BALL, color_id=protocol.COLOR_ANY),
			task_state.TargetConfig(slot_id=8, shape_id=protocol.SHAPE_BALL, color_id=protocol.COLOR_ANY),
		]
		first_view = [
			det("ball", "pink", 10),
			det("ball", "green", 40),
	...
	def test_advanced_2_2_more_than_two_matches_still_uses_current_leftmost(self):
		configs = [
			task_state.TargetConfig(slot_id=7, shape_id=protocol.SHAPE_CUBE, color_id=protocol.COLOR_ANY),
			task_state.TargetConfig(slot_id=8, shape_id=protocol.SHAPE_CUBE, color_id=protocol.COLOR_ANY),
		]
		items = [
			det("cube", "green", 80),
			det("cube", "pink", 15),
...
class CoordinateSendTests(unittest.TestCase):
	...
	def test_missing_placement_home_does_not_advance_and_retry_sends_same_target(self):
		...
		coordinate.frame_point_to_arm_coordinate = lambda frame_x, frame_y, placement, object_axis_angle_deg=None, shape=None: coordinate.CoordinateResult(
			x_cm=-3.30,
			y_cm=-4.10,
			x_raw=-330,
			y_raw=-410,
			base_angle_deg=-128.81,
			gripper_angle_deg=0.0,
		)
		try:
			runner._send_stable_target(target, controller)
			self.assertEqual(len(sent), 2)
			self.assertEqual(sent[0], bytes([0x04, 0x23, 0x28, 0x00, 0x00, 0x40]))
			self.assertEqual(sent[1], bytes([0x03, 0xFE, 0x52, 0xFE, 0xD4, 0x30]))
			self.assertEqual(controller.state, task_state.TaskState.WAIT_NEXT_HOME)
			self.assertEqual(controller.current_target_index, 0)
		finally:
			coordinate.frame_point_to_arm_coordinate = old_converter

	def test_arm_coordinate_offset_example(self):
		x_send = -5.78 + config.ARM_X_OFFSET_CM
		y_send = -13.92 + config.ARM_Y_OFFSET_CM
		self.assertAlmostEqual(x_send, -6.78, places=2)
		self.assertAlmostEqual(y_send, -12.82, places=2)
...
class AngleCalculationTests(unittest.TestCase):
	def test_requested_angle_case_negative_xy_long_axis(self):
		base = coordinate.calculate_base_angle_deg(-3.0, -3.0)
		gripper = coordinate.calculate_gripper_angle_deg(-90.0, base, "cuboid")
		self.assertAlmostEqual(base, -135.0, places=2)
		self.assertAlmostEqual(gripper, 45.0, places=2)

	def test_requested_angle_case_positive_y_long_axis(self):
		base = coordinate.calculate_base_angle_deg(0.0, 10.0)
		gripper = coordinate.calculate_gripper_angle_deg(45.0, base, "cuboid")
		self.assertAlmostEqual(base, 90.0, places=2)
		self.assertAlmostEqual(gripper, -45.0, places=2)

	def test_ball_gripper_angle_is_zero(self):
		base = coordinate.calculate_base_angle_deg(5.0, 5.0)
		gripper = coordinate.calculate_gripper_angle_deg(None, base, "ball")
		self.assertAlmostEqual(gripper, 0.0, places=2)

	def test_scene_detection_axis_uses_standard_plane_contour(self):
		try:
			import numpy as np
		except ModuleNotFoundError:
			self.skipTest("numpy is not installed in this Python environment")

		view = target_selector.AreaView(
			name="A",
			region=None,
			source_crop=None,
			valid_mask=None,
		...
		angle = target_selector._object_axis_angle_from_standard_contour(
			view,
			FakeDetection(),
		)
		self.assertAlmostEqual(angle, -90.0, places=2)

	def test_periodic_angle_stability_handles_wraparound(self):
		runner = vision_task.VisionTaskRunner()
		runner.target_history = [
			det("cuboid", "blue", 10),
			det("cuboid", "blue", 11),
			det("cuboid", "blue", 12),
		]
		runner.target_history[0].object_axis_angle_deg = 179.0
		runner.target_history[1].object_axis_angle_deg = -179.0
		runner.target_history[2].object_axis_angle_deg = 180.0
		angle = runner._stable_object_axis_angle()
		self.assertTrue(abs(abs(angle) - 180.0) < 2.0 or abs(angle) < 2.0)
